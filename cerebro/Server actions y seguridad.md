...
- **Lecturas también son RPC, y acotar por tenant NO es autorizar.** Un `get*` en `'use server'` es un endpoint. `getEventById` es lectura pública acotada al club… y durante un tiempo devolvió, dentro del evento, `attendances.member` **completo**: nombre, DNI, fecha de nacimiento, teléfono, dirección y datos del tutor de cada menor inscrito. Ninguna pantalla pintaba esa lista; salía solo por la invocación directa, con un POST sin sesión a la ruta pública del evento. La lección: en una lectura pública, la pregunta no es «¿de qué club es?» sino **«¿qué campos devuelve?»** — y la respuesta se decide con un `select` explícito, nunca con un `include` que crece solo cuando alguien añade una relación. Los `get*` de gestión (tienda, meta, bank-import) exigen rol.
